...
!! Compile with (willmb): mpif90 -cpp cic_power_v2.3.f90 -o cic_power -lsrfftw_mpi -lsrfftw -lsfftw_mpi -lsfftw -lm -ldl -DGFORTRAN -DPPINT -DIC -DWRITE_DEN
!! Compile with (ranger): mpif90 -fpp cic_power_v2.3.f90 -o cic_power -L$TACC_FFTW2_LIB -I$TACC_FFTW2_INC -lsrfftw_mpi -lsrfftw -lsfftw_mpi -lsfftw -lm -ldl -DPPINT -DBINARY -DIC -DWRITE_DEN
...
! frequently changed parameters are found in this header file:
	include './parameters_cubepm_110526_3_216_20Mpc'
  

	integer, parameter :: ncc = 432 ! GRID SIZE OF OUTPUT. Coarsening is then by factor of nc / ncc

  logical, parameter :: correct_kernel=.false.

#ifndef IC
  character(len=*), parameter :: checkpoints='./checkpoints'
  character(len=*), parameter :: offsets='./offsets_corrected.dat'
#endif

  !! ncc is the number of cells per coarsened box length
  integer, parameter :: hc=ncc/2
  real, parameter    :: ncr=ncc
  real, parameter    :: hcr=hc

  !! np is the number of particles
  !! np should be set to nc (1:1), hc (1:2), or qc (1:4)
  integer, parameter :: np=nc/2
  real, parameter    :: npr=np
#ifndef IC
  !! internals
  integer, parameter :: max_checkpoints=100
  real, dimension(max_checkpoints) :: z_checkpoint
  integer num_checkpoints, cur_checkpoint, num_offsets
  real, dimension(4,max_checkpoints) :: random_offset
  
#endif

  !! internal parallelization parameters
  integer(4), parameter :: nc_node_dim = ncc/nodes_dim
  ...
  integer(4), parameter :: nc_slab = ncc / nodes 
...
! :: simulation variables
  ...
  real, dimension(2,ncc) :: pkdm
#ifdef PLPLOT
  real*8, dimension(3,ncc) :: pkplot
#endif
  ...
  real, dimension(ncc+2,ncc,nc_slab) :: slab, slab_work

!    character(len=10) :: rank_string
!    character(len=400) :: fn
!	integer(4) nc_node_dim_test
...
#ifndef IC
  common z_checkpoint, num_checkpoints, cur_checkpoint
#endif
...
! FOR DEBUGGING OUTPUT IF REQUIRED: 

!if(rank .eq. 0) then ! testing the written data
  
!  print*, 'Testing written data'
!  write(rank_string,'(i7.3)') rank
!    rank_string=adjustl(rank_string)
  
!	fn=output_path//'IC_density_field_'&
!		&//rank_string(1:len_trim(rank_string))//'.dat'  
!  print*, 'Opening ',fn
  
!  open(13,file=fn,form='unformatted',access='stream')
  
!  read(13) nc_node_dim_test
!  read(13) cube(:,:,:)
!  print*, nc_node_dim_test
!  print*,cube(34,23,23),cube(8,2,4),cube(6,3,1),cube(4,4,4),cube(24,34,50)
...
!! set up global mpi communicator
    ...
    if (mod(nc,nodes) /= 0) then
      ...
      write(*,*) 'nc=',ncc,'nodes=',nodes,'mod(ncc,nodes) != 0'
      call mpi_abort(mpi_comm_world,ierr,ierr)
    endif
...
!! read in list of checkpoints to calculate spectra for
  ...
  51  close(11)
    ...
    do i = 1, num_checkpoints
    call mpi_bcast(z_checkpoint(i),1,mpi_integer,0,mpi_comm_world,ierr)
    enddo
...
#ifndef IC
  subroutine read_random_offsets
!! read in list of offsets
    implicit none

    integer :: i,j,fstat

    if (rank == 0) then
      open(11,file=offsets,status='old',iostat=fstat)
      if (fstat /= 0) then
        print *,'error opening random offsets list file'
        print *,'rank',rank,'file:',offsets
        call mpi_abort(mpi_comm_world,ierr,ierr)
      endif
      do num_offsets=1,max_checkpoints
!          read(unit=11,err=51,end=41,fmt='(f15.10,f15.10,f15.10,f15.10)') random_offset(1,num_offsets),&
          read(11,*,end=41) random_offset(1,num_offsets),&
          &random_offset(2,num_offsets),random_offset(3,num_offsets),random_offset(4,num_offsets)          
          print*, random_offset(:,num_offsets)
      enddo
41    num_offsets=num_offsets-1
51    close(11)
      print *,'random offsets:'
      do i=1,num_offsets
        write(*,'(4f15.10)') random_offset(:,i)
      enddo 
      
      if(num_offsets .ne. num_checkpoints) then
        print*,'mismatch in offsets versus checkpoints'
        print*, 'offsets = ',num_offsets,' checkpoints = ',num_checkpoints
        call mpi_abort(mpi_comm_world,ierr,ierr)
      endif
      
    endif

    call mpi_bcast(num_offsets,1,mpi_integer,0,mpi_comm_world,ierr)

    do i = 1, num_offsets
      do j = 1, 4
        call mpi_bcast(random_offset(j,i),1,mpi_integer,0,mpi_comm_world,ierr)
      enddo
    enddo

  end subroutine read_random_offsets
#endif
...
!! generate checkpoint names on each node
    if (rank==0) then
#ifdef IC
	print *,'calculating density field for ICs'
#else
	z_write = z_checkpoint(cur_checkpoint)
    print *,'calculating density field for z=',z_write
#endif 
    endif
...
#ifdef IC
	check_name=ic_path//'xv'// &
               rank_string(1:len_trim(rank_string))//'.ic'
#else
...
#else
#ifdef GFORTRAN
	open(unit=21,file=check_name,status='old',iostat=fstat,form='unformatted',access='stream')
#else
...
#else
#ifdef PPINT
    read(21) np_4bytes,a,t,tau,nts,dt_f_acc,dt_pp_acc,dt_c_acc,sim_checkpoint, &
               sim_projection,sim_halofind,mass_p
#else
    read(21) np_4bytes,a,t,tau,nts,dt_f_acc,dt_c_acc,sim_checkpoint, &
               sim_projection,sim_halofind,mass_p
#endif
#endif
...
!! tally up total number of particles
    call mpi_reduce(np_local,np_total,1,mpi_integer8, &
...
! adjust for random shifts:

  xvp(1,:) = xvp(1,:) - random_offset(2,cur_checkpoint)
  xvp(2,:) = xvp(2,:) - random_offset(3,cur_checkpoint)
  xvp(3,:) = xvp(3,:) - random_offset(4,cur_checkpoint)  

! recasts particle positions to the coarser grid
  
	xvp(1:3,:) = xvp(1:3,:) / (real(nc)/real(ncc) )
...
! initialize plan variables for fftw
    ...
    if (firstfftw) then
      call rfftw3d_f77_mpi_create_plan(plan,mpi_comm_world,ncc, &
            ncc,ncc, FFTW_REAL_TO_COMPLEX, FFTW_ESTIMATE)
      call rfftw3d_f77_mpi_create_plan(iplan,mpi_comm_world,ncc, &
            ncc,ncc, FFTW_COMPLEX_TO_REAL, FFTW_ESTIMATE)
#ifdef DEBUG_LOW
...
!!------------------------------------------------------------------!!
  ...
  subroutine writepowerspectra
    ...
    fn=z_write(1:len_trim(z_write))//'cicps.dat'
#endif
...
!!------------------------------------------------------------------!!
  ...
  subroutine write_density_field
    implicit none
	integer i, j,k, write_chunk_size, number_of_writes
    character*200 :: fn
    character*7  :: z_write, rank_string
    real time1,time2
    call cpu_time(time1)

	! calibrate for writing in chunks of less than 32MB for intel chips

	write_chunk_size = 1024*1024*24 !24 MB
	number_of_writes = nc_node_dim*nc_node_dim*nc_node_dim*8 / write_chunk_size + 1

#ifndef IC
	write(z_write,'(f7.3)') z_checkpoint(cur_checkpoint)
    z_write=adjustl(z_write)
    print*, 'writing density field for redshift',z_checkpoint(cur_checkpoint),&
    &'on task',rank
#endif
	...
	fn=output_path//'IC_density_field_'&
		&//rank_string(1:len_trim(rank_string))//'.dat'
#else
    fn=output_path//z_write(1:len_trim(z_write))//'_density_field_'&
		&//rank_string(1:len_trim(rank_string))//'.dat'
#endif   
    ...
    open(12,file=fn,form='binary')
	   ...
	   write(12) nc_node_dim

	if(nc_node_dim*nc_node_dim*nc_node_dim*8 .lt. write_chunk_size) then ! we can write entire density array in one go
		write(12) cube(:,:,:)

	if(rank .eq. 0) then
		print*, 'Writing whole density matrix'
	endif

	else if(nc_node_dim*nc_node_dim*8 .lt. write_chunk_size) then ! we can write an entire 2d plane of the density array in one go
		do i = 1, nc_node_dim
			write(12) cube(:,:,i)
		enddo

	if(rank .eq. 0) then
		print*, 'Writing individual planes'
	endif

	else if(nc_node_dim*8 .lt. write_chunk_size) then ! we can write a 1d strip of the density array in one go
		do j = 1, nc_node_dim
			do i = 1, nc_node_dim
				write(12) cube(:,i,j)
			enddo
	...
	if(rank .eq. 0) then
		print*, 'Writing individual strips'
	endif

	else if(nc_node_dim*8 .gt. write_chunk_size) then ! we have to split the 1d strips up into smaller chunks
	...
	if(rank .eq. 0) then
		print*, 'Writing individual elements'
	endif
	...
	open(12,file=fn,form='unformatted',access='stream')
    ...
    call cpu_time(time2)
    time2=time2-time1
    write(*,"(f8.2,a)") time2,'Called write density field'
    return
  end subroutine write_density_field
#endif
...
!!------------------------------------------------------------------!!
  ...
  subroutine darkmatter
    ...
    real*8  :: dsum,dvar,dsumt,dvart,sum_dm,sum_dm_local
    ...
    call mpi_reduce(sum_dm_local,sum_dm,1,mpi_real8,mpi_sum,0,mpi_comm_world,ierr)
    ...
    if (rank==0) then
      dsum=dsumt/real(ncc)**3
      dvar=sqrt(dvart/real(ncc)**3)
      write(*,*)
      write(*,*) 'DM min    ',dmint
      write(*,*) 'DM max    ',dmaxt
...
!------------------------------------------------------------!
  ...
  subroutine pass_particles
    ...
    integer*4 i
    integer*8 np_final,np_exit,np_buf,pp,npo,npi
    real x(3),lb,ub
...
#ifdef DEBUG
    do i=0,nodes-1
      if (rank==i) print *,rank,'original np_local=',np_local
      call mpi_barrier(mpi_comm_world,ierr)
    enddo
#endif 
    ...
    call mpi_reduce(np_buf,np_exit,1,mpi_integer8,mpi_sum,0, &
                    mpi_comm_world,ierr) 
...
! pass +x
...
#ifdef DEBUG
    do i=0,nodes-1
      if (rank==i) print *,rank,'np_out=',npo,'pp = ', pp
      call mpi_barrier(mpi_comm_world,ierr)
    enddo
#endif 
    ...
    call mpi_sendrecv_replace(npi,1,mpi_integer8,cart_neighbor(6), &
...
! pass -x
    ...
    call mpi_sendrecv_replace(npi,1,mpi_integer8,cart_neighbor(5), &
...
! pass +y
    ...
    call mpi_sendrecv_replace(npi,1,mpi_integer8,cart_neighbor(4), &
...
! pass -y
    ...
    call mpi_sendrecv_replace(npi,1,mpi_integer8,cart_neighbor(3), &
...
! pass +z
    ...
    call mpi_sendrecv_replace(npi,1,mpi_integer8,cart_neighbor(2), &
...
! pass -z
    ...
    call mpi_sendrecv_replace(npi,1,mpi_integer8,cart_neighbor(1), &
    ...
    call mpi_reduce(int(np_local,8),np_final,1,mpi_integer8,mpi_sum,0, &
                    mpi_comm_world,ierr)
    ...
    if (rank == 0) then
      ...
      if (np_final /= (int(np,8))**3) then
        ...
        print *,'expected particles = ',(int(np,8))**3
      endif
    endif
...
!!--------------------------------------------------------------!!
  ...
  subroutine powerspectrum(delta,pk)
    implicit none
    real, dimension(2,ncc)       :: pk
    real, dimension(ncc+2,ncc,nc_slab) :: delta

    integer :: i,j,k,kg
    integer :: k1,k2
    real    :: kr,kx,ky,kz,w1,w2,pow
    real, dimension(3,ncc,nc_slab) :: pkt
    real, dimension(3,ncc) :: pktsum
    ...
    !! Reduce to rank 0
    call mpi_reduce(pkt(:,:,1),pktsum,3*ncc,mpi_real,mpi_sum,0,mpi_comm_world,ierr)
    ...
    call mpi_bcast(pk,2*ncc,mpi_real,0,mpi_comm_world,ierr)
